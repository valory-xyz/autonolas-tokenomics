...
uniswapPriceOracleAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#uniswapPriceOracleAddress}
...
echo "${green}$contractName deployed at: $uniswapPriceOracleAddress${reset}"