...
optimismL2CrossDomainMessengerAddress=$(jq -r '.optimismL2CrossDomainMessengerAddress' $globals)
...
constructorArgs="$olasAddress $serviceStakingFactoryAddress $optimismL2CrossDomainMessengerAddress $optimismDepositProcessorL1Address $l1ChainId"
...
optimismTargetDispenserL2Address=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#optimismTargetDispenserL2Address}
...
echo "$(jq '. += {"optimismTargetDispenserL2Address":"'$optimismTargetDispenserL2Address'"}' $globals)" > $globals
# Also write the address into corresponding L1 JSON
echo "$(jq '. += {"optimismTargetDispenserL2Address":"'$optimismTargetDispenserL2Address'"}' $globalsL1)" > $globalsL1

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$optimismTargetDispenserL2Address $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address,address,uint256)" $constructorArgs)"
...
echo "${green}Contract deployed at: $optimismTargetDispenserL2Address${reset}"