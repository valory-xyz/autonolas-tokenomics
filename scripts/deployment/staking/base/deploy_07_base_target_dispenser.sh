...
baseL2CrossDomainMessengerAddress=$(jq -r '.baseL2CrossDomainMessengerAddress' $globals)
...
constructorArgs="$olasAddress $serviceStakingFactoryAddress $baseL2CrossDomainMessengerAddress $baseDepositProcessorL1Address $l1ChainId"
...
baseTargetDispenserL2Address=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#baseTargetDispenserL2Address}
...
echo "$(jq '. += {"baseTargetDispenserL2Address":"'$baseTargetDispenserL2Address'"}' $globals)" > $globals
# Also write the address into corresponding L1 JSON
echo "$(jq '. += {"baseTargetDispenserL2Address":"'$baseTargetDispenserL2Address'"}' $globalsL1)" > $globalsL1

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$baseTargetDispenserL2Address $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address,address,uint256)" $constructorArgs)"
...
echo "${green}Contract deployed at: $baseTargetDispenserL2Address${reset}"