...
polygonFXChildAddress=$(jq -r '.polygonFXChildAddress' $globals)
...
polygonTargetDispenserL2Address=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#polygonTargetDispenserL2Address}
...
echo "$(jq '. += {"polygonTargetDispenserL2Address":"'$polygonTargetDispenserL2Address'"}' $globals)" > $globals
# Also write the address into corresponding L1 JSON
echo "$(jq '. += {"polygonTargetDispenserL2Address":"'$polygonTargetDispenserL2Address'"}' $globalsL1)" > $globalsL1

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$polygonTargetDispenserL2Address $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address,address,uint256)" $constructorArgs)"
...
echo "${green}Contract deployed at: $polygonTargetDispenserL2Address${reset}"