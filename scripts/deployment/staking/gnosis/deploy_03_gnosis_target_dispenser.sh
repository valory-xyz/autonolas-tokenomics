...
contractPath="contracts/staking/GnosisTargetDispenserL2.sol:GnosisTargetDispenserL2"
constructorArgs="$olasAddress $serviceStakingFactoryAddress $gnosisAMBHomeAddress $gnosisDepositProcessorL1Address $l1ChainId"
...
gnosisTargetDispenserL2Address=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#gnosisTargetDispenserL2Address}
...
echo "$(jq '. += {"gnosisTargetDispenserL2Address":"'$gnosisTargetDispenserL2Address'"}' $globals)" > $globals
# Also write the address into corresponding L1 JSON
echo "$(jq '. += {"gnosisTargetDispenserL2Address":"'$gnosisTargetDispenserL2Address'"}' $globalsL1)" > $globalsL1

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$gnosisTargetDispenserL2Address $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address,address,uint256)" $constructorArgs)"
...
echo "${green}Contract deployed at: $gnosisTargetDispenserL2Address${reset}"