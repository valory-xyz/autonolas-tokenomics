...
lpSwapMaxSlippage=$(jq -r '.lpSwapMaxSlippage' $globals)

contractName="LPSwapCelo"
contractPath="contracts/utils/$contractName.sol:$contractName"
constructorArgs="$uniswapPriceOracleAddress $lpSwapMaxSlippage"
...
lpSwapCeloAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#lpSwapCeloAddress}
...
echo "$(jq '. += {"lpSwapCeloAddress":"'$lpSwapCeloAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$lpSwapCeloAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,uint256)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $lpSwapCeloAddress${reset}"
