...
constructorArgs="$buyBackBurnerAddress $proxyData"
...
buyBackBurnerProxyAddress=$(echo "$deploymentOutput" | grep 'Deployed to:' | awk '{print $3}')

# Get output length
outputLength=${#buyBackBurnerProxyAddress}
...
echo "$(jq '. += {"buyBackBurnerProxyAddress":"'$buyBackBurnerProxyAddress'"}' $globals)" > $globals

# Verify contract
if [ "$contractVerification" == "true" ]; then
  contractParams="$buyBackBurnerProxyAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,bytes)" $constructorArgs)"
...
echo "${green}$contractName deployed at: $buyBackBurnerProxyAddress${reset}"