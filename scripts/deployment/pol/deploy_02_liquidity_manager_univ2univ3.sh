...
constructorArgs="$olasAddress $treasuryAddress $positionManagerV3Address $neighborhoodScannerAddress $observationCardinality $routerV2Address"
...
if [ "$contractVerification" == "true" ]; then
  contractParams="$liquidityManagerAddress $contractPath --constructor-args $(cast abi-encode "constructor(address,address,address,address,uint16,address)" $constructorArgs)"
