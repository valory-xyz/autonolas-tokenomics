...
contract TreasuryMerkle is IErrorsTokenomics {
    ...
    event DonateToServicesETH(address indexed sender, uint256 indexed roundId, uint256[] serviceIds, uint256[] amounts,
        bytes32[] serviceMerkleRoots, bytes32[] serviceIPFSHashes);
    ...
    function depositServiceDonationsETH(
        ...
        uint256[] memory amounts,
        bytes32[] memory serviceMerkleRoots,
        bytes32[] memory serviceIPFSHashes
    ) external payable {
        // Reentrancy guard
        if (_locked > 1) {
        ...
        if (numServices == 0 && amounts.length != numServices || serviceMerkleRoots.length != numServices ||
            serviceIPFSHashes.length != numServices) {
            revert WrongArrayLength(numServices, amounts.length);
        }

        uint256 totalAmount;
        for (uint256 i = 0; i < numServices; ++i) {
            if (amounts[i] == 0 || serviceMerkleRoots[i] == 0 || serviceIPFSHashes[i] == 0) {
                revert ZeroValue();
            }
        ...
        uint256 roundId = ITokenomicsMerkle(tokenomics).trackServiceDonations(msg.sender, serviceIds, amounts,
            serviceMerkleRoots, msg.value);

        emit DonateToServicesETH(msg.sender, roundId, serviceIds, amounts, serviceMerkleRoots, serviceIPFSHashes);
