...
contract TokenomicsMerkle is TokenomicsConstants, IErrorsTokenomics {
    ...
    /// @param serviceIds Set of service Ids.
    /// @param amounts Correspondent set of ETH amounts provided by services.
    /// @param serviceMerkleRoots Set of Merkle tree roots corresponding to each serviceId.
    /// @param donationETH Overall service donation amount in ETH.
    /// @return roundId Recorded donation round Id.
    ...
    function trackServiceDonations(
        ...
        bytes32[] memory serviceMerkleRoots,
        uint256 donationETH
        ...
        for (uint256 i = 0; i < serviceIds.length; ++i) {
            ...
            // functions to check each service owner veOLAS balance
            bool topUpEligible;
            // Check the unit top-up fractions, and if at least one is non-zero, check for the top-up eligibility
            if (mapEpochTokenomics[curEpoch].unitPoints[0].topUpUnitFraction > 0 ||
            ...
            // Record top-up eligibility in the component field (could be agent one, it does not matter)
            if (topUpEligible) {
                    mapEpochTokenomics[curEpoch].unitPoints[0].sumUnitTopUpsOLAS += uint96(amounts[i]);
            }

            // Record the (service Id, round Id) data
    ...
    ) external returns (uint256 reward, uint256 topUp)
    {
        ...
        for (uint256 r = 0; r < roundIds.length; ++r) {
            ...
            for (uint256 i = 0; i < claims[r].length; ++i) {
                ...
                // Calculate OLAS topUps based on the reward amount
                if (rInfo.topUpEligible) {
                    //topUps[claims[r][i][0]] += claims[r][i][2];
    ...
    /// @dev Records service donations into corresponding data structures.
    /// @param serviceIds Set of service Ids.
    /// @return unitIds 3D set of components / agents corresponding to provided serviceIds.
    function getDonationsData(uint256[] memory serviceIds) external view returns (uint32[][][] memory unitIds) {
        // Component / agent registry addresses
        address[] memory registries = new address[](2);
        (registries[0], registries[1]) = (componentRegistry, agentRegistry);
        ...
        // Get the number of services
        uint256 numServices = serviceIds.length;
        // Allocate the number of unit related arrays
        unitIds = new uint32[][][](2);
        unitIds[0] = new uint32[][](numServices);
        unitIds[1] = new uint32[][](numServices);
        // Loop over service Ids to calculate their partial contributions
        for (uint256 i = 0; i < numServices; ++i) {
            // Loop over component and agent Ids
            for (uint256 unitType = 0; unitType < 2; ++unitType) {
                ...
                (numServiceUnits, unitIds[unitType][i]) = IServiceRegistry(serviceRegistry).
                    getUnitIdsOfService(IServiceRegistry.UnitType(unitType), serviceIds[i]);
                // Service has to be deployed at least once to be able to receive donations,
                // otherwise its components and agents are undefined
                if (numServiceUnits == 0) {
