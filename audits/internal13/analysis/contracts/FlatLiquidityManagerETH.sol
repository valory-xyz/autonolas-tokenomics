...
abstract contract LiquidityManagerCore is ERC721TokenReceiver {
    ...
    constructor(
        ...
        factoryV3 = IUniswapV3_0(positionManagerV3).factory();
    }
    ...
    {
        // Get slot0 payload
        bytes memory payload = abi.encodeCall(IUniswapV3_0.slot0, ());
    ...
    function _collectFees(uint256 positionId) internal returns (uint256[] memory amounts) {
        IUniswapV3_0.CollectParams memory params = IUniswapV3_0.CollectParams({
        ...
        (amounts[0], amounts[1]) = IUniswapV3_0(positionManagerV3).collect(params);
    ...
    function convertToV3(
        ...
        if (positionId == 0) {
            ...
            IUniswapV3_0(v3Pool).increaseObservationCardinalityNext(observationCardinality);
        } else {
    ...
    function getTwapFromOracle(address pool) public view returns (uint256 price, uint160 centerSqrtPriceX96) {
        ...
        (int56[] memory tickCumulatives,) = IUniswapV3_0(pool).observe(secondsAgo);
    ...
    function checkPoolAndGetCenterPrice(address pool) public view returns (uint160 centerSqrtPriceX96) {
        ...
        (uint32 oldestTimestamp,,,) = IUniswapV3_0(pool).observations(observationIndex);
...
contract LiquidityManagerETH is LiquidityManagerCore {
        ...
        returns (uint256[] memory amounts)
    {
        ...
        uint256 liquidity = IToken_1(lpToken).balanceOf(address(this));
        ...
        IToken_1(lpToken).approve(routerV2, liquidity);
    ...
    function _mintV3(
        ...
        // Params for minting
        IUniswapV3_1.MintParams memory params = IUniswapV3_1.MintParams({
            token0: tokens[0],
            token1: tokens[1],
            fee: uint24(feeTier),
            tickLower: ticks[0],
        ...
        // Mint position
        (positionId, liquidity, amounts[0], amounts[1]) = IUniswapV3_1(positionManagerV3).mint(params);
