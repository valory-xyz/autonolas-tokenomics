...
pragma solidity >=0.8.0 ^0.8.0 ^0.8.28 ^0.8.30;
...
abstract contract BuyBackBurner {
    ...
    function buyBack(address secondToken, uint256 secondTokenAmount) external virtual {
        ...
        uint256 balance = IERC20_0(localSecondToken).balanceOf(address(this));
        ...
        olasAmount = IERC20_0(olas).balanceOf(address(this));

        // Transfer OLAS to bridge2Burner contract
        IERC20_0(olas).transfer(bridge2Burner, olasAmount);
    ...
    function transfer(address token) external {
        ...
        // Get token amount
        uint256 tokenAmount = IERC20_0(token).balanceOf(address(this));
        ...
        if (token == olas) {
            ...
            IERC20_0(olas).transfer(bridge2Burner, tokenAmount);

            // Correct to value
            to = bridge2Burner;
        } else {
            // Transfer token to treasury contract
            IERC20_0(token).transfer(treasury, tokenAmount);
        }
...
contract BuyBackBurnerBalancer is BuyBackBurner {
        ...
        returns (uint256 olasAmount)
    {
        ...
        IERC20_1(secondToken).approve(balVault, secondTokenAmount);
