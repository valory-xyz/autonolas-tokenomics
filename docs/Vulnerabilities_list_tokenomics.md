...
2. **Keeps exits and fee collection live** — the one structural consequence of (1). `decreaseLiquidity` does not use the fail-closed guard; instead it derives its slippage floor **at execution** from a *soft-priced* fair value (`_getExitSqrtPrice`: the 30-minute TWAP when the pool is verifiable, with `slot0` bounded to it within `MAX_ALLOWED_DEVIATION`, otherwise the raw `slot0` fail-open). So a withdrawal is always possible even on a quiet pool, the floor never goes stale across a governance delay, and on a mature pool a >10% slot0 manipulation still reverts. `collectFees` likewise drops the guard gate and burns only the just-collected fee.
