...
| `[FIX-2b]` | `decreaseLiquidity` gated by the shared guard; `amountMin` derived from raw `slot0` | Shared guard dropped; `_getExitSqrtPrice` reverts if `slot0` is >`MAX_ALLOWED_DEVIATION` off the TWAP on a verifiable pool (anti-manipulation gate), else skips the gate (quiet / fresh), and in both cases returns raw `slot0`; `amountMin = amounts(slot0) × (1 − maxSlippage)` is computed at the same price the position manager withdraws at, so a fair exit is always satisfiable within the gate and never stale across a governance delay |
