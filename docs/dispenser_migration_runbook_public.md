...
14. On the **new** L2 dispenser, `updateWithheldAmountMaintenance(...)` to re-establish its `withheldAmount` (it deploys at 0). **Which value to pass depends on the Phase 0 reading, and this is the easiest step to get wrong:**
    - **L1 read `0` for this chain** → pass the **emitted `withheldAmount`** from `Migrated`. The accounting still lives on L2 and this restores it unchanged.
    - **L1 read non-zero for this chain** → the emitted `withheldAmount` is **`0`** (the accounting moved to L1 when it was synced), while `migrate()` still carried a non-zero OLAS **balance**. Passing `0` here leaves that OLAS accounted for by neither side: the new L1 Dispenser also starts at `0`, nothing nets against it, and the new L2 dispenser quietly accumulates unusable OLAS. Pass the **migrated balance** from `Migrated` instead, so step 22 can carry it up.
    - *Alternative for the non-zero case:* leave this at `0` and restore the L1 side directly with `Dispenser.syncWithheldAmountMaintenance(chainId, amount, batchHash)` **after step 19** (it resolves the processor via `mapChainIdDepositProcessors` and calls `updateHashMaintenance`). That consumes a batch hash on the new processor, so prefer the L2 route unless the interaction with the new L2 dispenser's first genuine sync has been checked.
...
- **On-chain state (Phase 0.1):** read `mapChainIdWithheldAmounts` for every chain, plus each L2 dispenser's `withheldAmount` **and** its OLAS balance. A non-zero L1 value is covered — see the step 14 note — but it changes which value step 14 restores, and getting that wrong strands the carried OLAS on the new L2 dispenser.
