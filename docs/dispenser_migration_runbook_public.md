...
22. **Withheld re-sync — no special step, but confirm it happens.** With step 14 done correctly the carried amount is on the new L2 dispenser's `withheldAmount`, and the ordinary `syncWithheldTokens` path carries it up to the new L1 Dispenser like any other sync; nothing migration-specific is required. What is worth checking is that it *has* happened before the first full epoch distribution, since until it does the new Dispenser reads `mapChainIdWithheldAmounts = 0` and bridges fresh OLAS instead of netting (funds are not lost, the L2 simply stays "ahead"). A chain left at `0` by step 14 cannot be repaired here — the sync path rejects a zero amount — so any shortfall points back to step 14, not to this step.
